...
function Navbar({setHambState, hambState}) {
    ...
    return (
      ...
      <section className="navbar animate__animated animate__fadeInDown animate__faster py-4  sm:px-5 bg-sky-200 border-b-2 border-sky-300 shadow-md shadow-sky-200/60">
          <div className="flex justify-between items-center">
            ...
            {/* logo  */}
            <div className={`flex flex-1 items-center justify-between   ${ hambState==='close'? '' : 'blur-md'} `}>
            ...
            <div>
              ...
              <div className={` ${ hambState==='close'? '' : 'backdrop-blur-3xl h-screen w-screen absolute top-0 right-0 touch-none'}`}></div>
              ...
              <div id=""
                ...
                {/* menu button */}
                <button name='hambtn' title='menu' id="hambtn" className={`relative text-3xl cursor-pointer mx-2 xl:hidden block ${ hambState==='close'? '' : 'z-30 ' }`} onClick={(e)=>handleMenu(e)}>
                  <CgMenuRight className={`w-9 h-9 ${ hambState==='close'? 'stroke-slate-400' : 'hidden' }`}/>
                  <IoClose className={` w-9 h-9  stroke-1 ${ hambState==='close'? 'hidden ' : 'fill-slate-100/80 animate__animated animate__rotateIn animate__faster' }`}/>
                </button>
  
                {/* menu */}
                <div className={`h-screen xl:hidden fixed font-calibre text-slate-50  bg-sky-800/80 border-l border-l-lightestnavy/80 flex flex-col justify-center items-center  gap-10   ${ hambState==='close'? 'hidden top-0 right-0 ' : 'w-2/3 z-20  top-0 right-0 touch-none duration-200 ease-in-out transition-all' } `}>
